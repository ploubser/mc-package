...
#
# Basic implementaion of deb package creation.
# NOTE: Debian and Redhat package classes will eventually become one package
# to avoid all the code duplication.
module MCPackage
  class Debian < Package
    ...
    # Options common to all type of deb packages created by fpm
    def standard_flags(dir = "common")
      params = ["-s", "dir", "-C", @tmp_dir, "-t", "deb", "-a", "all", "-n", "mcollective-#{@name}-#{dir}", "-v",
